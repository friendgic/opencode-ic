...
describe("sanitizeOpenAICompatibleChatCompletionsBody", () => {
  test("sets assistant content to null when empty string and tool_calls present", () => {
    const body = {
      model: "claude",
      messages: [
    ...
    ProviderTransform.sanitizeOpenAICompatibleChatCompletionsBody(body)
    expect(body.messages[1].content).toBeNull()
  })

  test("does not change assistant with empty content and no tool_calls", () => {
    const body = {
  ...
  test("does not change non-empty assistant content", () => {
    const body = {
      messages: [
        {
          role: "assistant",
  ...
  test("no-ops on invalid shapes", () => {
    ProviderTransform.sanitizeOpenAICompatibleChatCompletionsBody(null)
    ProviderTransform.sanitizeOpenAICompatibleChatCompletionsBody({ messages: "no" })
  })
})

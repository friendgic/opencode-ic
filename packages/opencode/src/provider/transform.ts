...
/**
 * @ai-sdk/openai-compatible emits `content: ""` for assistant turns that only contain tool calls.
 * Anthropic-backed gateways (e.g. One API → Claude) reject empty text blocks; OpenAI-style payloads
 * accept `content: null` for those turns instead.
 */
export function sanitizeOpenAICompatibleChatCompletionsBody(body: unknown): void {
